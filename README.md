...
A comparative analysis showed MediaPipe to be objectively superior to Dlib for this real-time application. MediaPipe provides a 2.5x higher framerate and, most importantly, does not fail when the head is turned, unlike Dlib's "frontal-only" detector.

Metric

Dlib (Baseline)

MediaPipe (This Project)
...
"Frontal-only" detector. Fails at slight angles (~20° Yaw).

Tracks full range of motion (~355° Yaw).
